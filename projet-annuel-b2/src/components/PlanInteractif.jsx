...
export default function PlanInteractif({ etage = 'RDC', filtreStatut = 'Tous', filtreProbleme = 'Tous' }) {
  ...
  return (
    <div className="relative w-full max-w-2xl h-64 bg-gray-100 rounded shadow mb-4">
      {/* Fond du plan (remplacer par SVG ou image réelle si besoin) */}
      <div className="absolute inset-0 flex items-center justify-center text-gray-400 text-6xl opacity-10">
        PLAN {etage}
      </div>
      {/* Salles positionnées dynamiquement */}
      {sallesEtage.map(salle => {
        ...
        return (
          <button
            key={salle.id}
            className={`absolute px-3 py-1 rounded cursor-pointer font-semibold shadow-lg border-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-150 ${statutDominant ? getStatutCouleur(statutDominant) + ' text-white' : 'bg-gray-200 text-gray-800'}`}
